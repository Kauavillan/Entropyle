...
describe("GET /api/words", () => {
  ...
  it("applies the top100 pool when validating a word", async () => {
    const words = Array.from({ length: 101 }, (_, index) =>
      `w${String(index + 1).padStart(4, "0")}`,
    );
    jest.spyOn(fs, "readFile").mockResolvedValueOnce(words.join("\n") as never);

    const response = await GET(
      new Request(
        "http://localhost/api/words?length=5&word=w0101&pool=top100",
      ),
    );
  ...
  it("applies the top300 pool when validating a word", async () => {
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/words?length=5&word=w0001&pool=top300",
      ),
    );
  ...
  it("returns generated words with the requested count", async () => {
    jest.spyOn(fs, "readFile").mockResolvedValueOnce("abca\nbcdb\ncdce\n" as never);
